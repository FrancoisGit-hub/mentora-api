...
public class ProgramTemplateRequestValidator : AbstractValidator<ProgramTemplateRequest>
{
    ...
    public ProgramTemplateRequestValidator(IExerciseService exerciseService)
    {
        ...
        RuleFor(x => x).CustomAsync(ValidateBodyAsync);
    }

    // ── Body tree ──────────────────────────────────────────────────────────────

    private async Task ValidateBodyAsync(
        ProgramTemplateRequest request, ValidationContext<ProgramTemplateRequest> context, CancellationToken ct)
    {
        if (request.Body?.Blocks is not { Count: > 0 })
        {
            context.AddFailure("Body.Blocks", "At least one block is required.");
            return;
        }
        ...
        ValidateBlockList(request.Body.Blocks, parentRank: 0, depth: 1, "Body.Blocks", context, weekNumbers, exerciseIds);
        ValidateWeekNumbers(weekNumbers, request.DurationWeeks, context);
        ...
        // exercises undetected. This is the barrier Lot 6.3 will reuse during program assignment.
        if (!context.RootContextData.TryGetValue("CoachId", out var coachIdValue) || coachIdValue is not Guid coachId)
            throw new InvalidOperationException(
                "ProgramTemplateRequestValidator requires ValidationContext.RootContextData[\"CoachId\"] " +
        ...
        if (missing.Count > 0)
            context.AddFailure("Body", $"Unknown or inaccessible exercise id(s): {string.Join(", ", missing)}.");
    }

    private void ValidateBlockList(
        List<ProgramTemplateBlock> blocks, int parentRank, int depth, string path,
        ValidationContext<ProgramTemplateRequest> context, List<int> weekNumbers, HashSet<Guid> exerciseIds)
    {
    ...
    private static void ValidateWeekNumbers(
        List<int> weekNumbers, int durationWeeks, ValidationContext<ProgramTemplateRequest> context)
    {
        var distinct = weekNumbers.Distinct().OrderBy(w => w).ToList();

        if (distinct.Count != weekNumbers.Count)
            context.AddFailure("Body", "WeekNumber values must be unique across all MICROCYCLE blocks.");

        var expected = Enumerable.Range(1, durationWeeks).ToList();
        if (!distinct.SequenceEqual(expected))
            context.AddFailure("Body",
    ...
    private void ValidateSessions(
        ...
        ValidationContext<ProgramTemplateRequest> context, HashSet<Guid> exerciseIds)
    {
    ...
    private static void ValidateSessionType(
        string? type, string path, ValidationContext<ProgramTemplateRequest> context)
    {
    ...
    private static void ValidatePositions<T>(
        List<T> items, Func<T, int> selector, string path, ValidationContext<ProgramTemplateRequest> context)
    {
